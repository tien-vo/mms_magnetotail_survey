...
def combine_omni(interval, species="ion", N_ext=5, mask_cutoff=True, bg_remove=False, factor=1):
    ...
    t_fpi = read_data(f"mms1/{species}-fpi-moms/interval_{interval}/t").astype("datetime64[ns]")
    f_fpi = read_data(f"mms1/{species}-fpi-moms/interval_{interval}/f_omni")
    E_fpi = read_data(f"mms1/{species}-fpi-moms/interval_{interval}/f_omni_energy")
    ...
    t_feeps = read_data(f"mms1/{species}-feeps/interval_{interval}/t").astype("datetime64[ns]")
    f_feeps = read_data(f"mms1/{species}-feeps/interval_{interval}/f_omni")
    E_feeps = np.tile(read_data(f"mms1/{species}-feeps/interval_{interval}/f_omni_energy"), (t_fpi.shape[0], 1))
    # NOTE: recalculate the 2/3rd average
    window = 2 / 3 * (19.67 * u.s / tv.numeric.sampling_period(t_feeps)).decompose()
    ...
    h5f = h5.File(lib.postprocess_dir / f"interval_{interval}.h5", "w")
