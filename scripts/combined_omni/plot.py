...
def plot(interval):
    t_ion = read_data(f"/postprocess/interval_{interval}/ion/t").astype(
        "datetime64[ns]"
    )
    tg_ion = read_data(f"/postprocess/interval_{interval}/ion/tg").astype(
        "datetime64[ns]"
    )
    ...
    t_elc = read_data(f"/postprocess/interval_{interval}/elc/t").astype(
        "datetime64[ns]"
    )
    tg_elc = read_data(f"/postprocess/interval_{interval}/elc/tg").astype(
        "datetime64[ns]"
    )
    ...
    im = ax.pcolormesh(
        tg_ion, E_ion.value, f_ion.value, cmap="jet", norm=mu.mplc.LogNorm()
    )
    ...
    im = ax.pcolormesh(
        tg_ion, E_elc.value, f_elc.value, cmap="jet", norm=mu.mplc.LogNorm()
    )
    fig.colorbar(im, cax=cax)
    ...
    for i, ax in enumerate(axes):
        ax.set_xlim(
            np.datetime64("2017-07-26T07:10"),
            np.datetime64("2017-07-26T07:50"),
        )
